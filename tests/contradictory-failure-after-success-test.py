#!/usr/bin/env python3
"""Bounded proof that contradictory FAILED cannot regress terminal EXECUTED."""
...
def wait_for_dlt_increment(before: int) -> int:
    ...
    while time.monotonic() < deadline:
        ...
        time.sleep(helpers.POLL_INTERVAL_SECONDS)
    raise helpers.CommandLifecycleFailure(
        f"contradictory FAILED did not increment terra.control.feedback.DLT; before={before} latest={latest}"
    )


def main() -> int:
    run_id = uuid.uuid4().hex[:10]
    farm_id = f"farm-successguard-{run_id}"
    asset_id = f"fan-successguard-{run_id}"
    plan_id = f"plan-{run_id}"
    trace_id = f"trace-successguard-{run_id}"
    ...
    event = {
        ...
        "data": {
            ...
            "reasoning": "contradictory terminal failure ordering proof", "requires_approval": True,
    ...
    approval = helpers.requests.post(
        ...
        json={"notes": "contradictory failure proof approval"},
    ...
    print("[4/8] Establish delivery and complete with correlated MQTT EXECUTED")
    ...
    helpers.publish_mqtt(status_topic, {
        ...
        "lastCommandId": command_id, "lastCommandStatus": "EXECUTED", "lastCommandError": "",
    ...
    if executed.get("commandId") != command_id or executed.get("executionResult") != "DEVICE_CONFIRMED":
        raise helpers.CommandLifecycleFailure(f"command did not reach expected terminal success: {executed}")
    stable = {key: executed.get(key) for key in STABLE_FIELDS}

    print("[5/8] Record DLT end offset before contradictory terminal feedback")
    dlt_before = dlt_end_offset()

    print("[6/8] Publish schema-valid correctly correlated FAILED for terminal EXECUTED command")
    ...
    publish_feedback({
        ...
        "data": {
            ...
            "farm_id": farm_id, "target_asset_id": asset_id, "status": "FAILED",
            "error": "synthetic contradictory terminal failure", "timestamp": now,
        },
    })

    print("[7/8] Require contradictory FAILED to traverse configured rejection path")
    dlt_after = wait_for_dlt_increment(dlt_before)
    print(f"  PASS DLT offset advanced from {dlt_before} to {dlt_after}")

    print("[8/8] Verify terminal success fields remain unchanged")
    ...
    if final_fields != stable:
        raise helpers.CommandLifecycleFailure(
            f"contradictory FAILED mutated terminal success: before={stable} after={final_fields}"
        )
    print(f"PASS terminal EXECUTED truth preserved for command={command_id}")
    print("CONTRADICTORY FAILURE AFTER SUCCESS PROOF PASS")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"CONTRADICTORY FAILURE AFTER SUCCESS PROOF FAIL: {exc}", file=sys.stderr)
        raise SystemExit(1)
