#!/usr/bin/env python3
"""Bounded synthetic proof for terminal ACK recovery across a Kafka broker restart."""
...
def wait_for_recovered_terminal(plan_id: str, token: str, command_id: str) -> Dict[str, Any]:
    deadline = time.monotonic() + cl.POLL_TIMEOUT_SECONDS
    latest: Dict[str, Any] = {}
    while time.monotonic() < deadline:
        latest = cl.response_json(
            cl.fetch_plan(plan_id, token), "Kafka restart recovery action plan query"
        )
        ...
        if status in {
            ...
            raise cl.CommandLifecycleFailure(
                f"Kafka restart recovery entered terminal failure state: {latest}"
            )
        time.sleep(cl.POLL_INTERVAL_SECONDS)
    raise cl.CommandLifecycleFailure(
        f"timed out waiting for Kafka restart ACK recovery; latest={latest}"
    )
...
def main() -> int:
    ...
    farm_id = f"farm-kafka-{run_id}"
    asset_id = f"fan-kafka-{run_id}"
    plan_id = f"plan-{run_id}"
    trace_id = f"trace-kafka-{run_id}"
    ...
    cl.publish_action_plan({
        ...
        "data": {
            ...
            "reasoning": "Synthetic Kafka restart ACK recovery proof",
    ...
    approval = cl.requests.post(
        ...
        json={"notes": "Kafka restart ACK recovery proof"},
    ...
    print("[6/10] Stop only Kafka broker")
    result = compose("stop", "kafka", check=False)
    if result.returncode != 0:
        raise cl.CommandLifecycleFailure(f"failed to stop kafka: {result.stdout}")

    print("[7/10] Publish correlated terminal ACK through MQTT while Kafka is down")
    ...
    if consumed_state.get("lastCommandId") != command_id:
        raise cl.CommandLifecycleFailure(f"Terra-Sense MQTT ACK correlation mismatch: {consumed_state}")

    print("[8/10] Restart Kafka and require broker readiness")
    result = compose("up", "-d", "kafka", check=False)
    if result.returncode != 0:
        raise cl.CommandLifecycleFailure(f"failed to restart kafka: {result.stdout}")
    wait_for_kafka_ready()

    print("[9/10] Verify buffered/retried ACK reconciles the persisted same plan")
    ...
    if recovered.get("executionResult") not in {"DEVICE_CONFIRMED", "DEVICE_CONFIRMED_LATE"}:
        raise cl.CommandLifecycleFailure(f"Kafka restart ACK recovery result mismatch: {recovered}")

    print("[10/10] Replay terminal ACK after broker recovery and verify idempotency")
    replay = dict(ack)
    ...
    if replay_state.get("lastCommandId") != command_id or replay_state.get("lastCommandStatus") != "EXECUTED":
        raise cl.CommandLifecycleFailure(f"replayed Kafka restart ACK state mismatch: {replay_state}")
    time.sleep(cl.POLL_INTERVAL_SECONDS * 2)
    terminal = cl.response_json(cl.fetch_plan(plan_id, token), "Kafka restart ACK replay plan query")
    if terminal.get("status") != "EXECUTED" or terminal.get("commandId") != command_id:
        raise cl.CommandLifecycleFailure(f"replayed Kafka restart ACK regressed terminal plan: {terminal}")
    if terminal.get("executionResult") != recovered.get("executionResult"):
        raise cl.CommandLifecycleFailure(
            f"replayed Kafka restart ACK changed execution result: before={recovered} after={terminal}"
        )

    print("PASS terminal ACK recovery across Kafka restart remains bounded synthetic software evidence")
    return 0
