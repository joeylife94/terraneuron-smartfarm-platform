#!/usr/bin/env python3
"""Bounded proof that contradictory EXECUTED cannot regress terminal DELIVERY_FAILED."""
...
def main() -> int:
    ...
    farm_id = f"farm-deliveryguard-{run_id}"
    asset_id = f"fan-deliveryguard-{run_id}"
    plan_id = f"plan-{run_id}"
    trace_id = f"trace-deliveryguard-{run_id}"
    ...
    event = {
        ...
        "data": {
            ...
            "reasoning": "terminal delivery failure contradiction proof", "requires_approval": True,
    ...
    approval = helpers.requests.post(
        ...
        json={"notes": "delivery failure contradiction proof approval"},
    ...
    print("[6/9] Record DLT end offset before contradictory terminal feedback")
    dlt_before = dlt_end_offset()

    print("[7/9] Publish schema-valid correctly correlated EXECUTED for terminal DELIVERY_FAILED command")
    ...
    print("[8/9] Require contradictory EXECUTED to traverse configured rejection path")
    ...
    if final_fields != stable:
        raise helpers.CommandLifecycleFailure(
            f"contradictory EXECUTED mutated terminal delivery failure: before={stable} after={final_fields}"
        )
    print(f"PASS terminal DELIVERY_FAILED truth preserved for command={command_id}")
    print("CONTRADICTORY SUCCESS AFTER DELIVERY FAILURE PROOF PASS")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"CONTRADICTORY SUCCESS AFTER DELIVERY FAILURE PROOF FAIL: {exc}", file=sys.stderr)
        raise SystemExit(1)
